...
(function() {
  var Connection, Db, ObjectID, Server, dbName, hash, loadLeagueInternal, withCollection;
  ...
  loadLeagueInternal = function(coll, id, fn) {
    return coll.findOne({
      _id: new ObjectID(id)
    }, function(err, league) {
      if (err) {
  ...
  exports.loadLeague = function(id, fn) {
    return withCollection("leagues", function(coll) {
      return loadLeagueInternal(coll, id, fn);
    });
  };

  exports.addFriend = function(leagueId, friend, fn) {
    return withCollection("leagues", function(coll) {
      return loadLeagueInternal(coll, id, function(err, league) {
        if (err) {
          return fn(err);
        }
        if (!league.friends) {
        ...
        return coll.save(league, function(err, doc) {
          return fn(err, doc);
        });
      });
    });
